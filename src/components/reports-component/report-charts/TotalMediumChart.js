    ...
    const unsubscribe = onSnapshot(q, (querySnapshot) => {
      ...
      querySnapshot.forEach((doc) => {
        ...
        tempEggsData.push({
          ...
          mediumCollected: eggData.medium,
        });
      });
      setMediumData(tempEggsData);
    });

    return () => unsubscribe();
  }, [currentUser]);

  const formattedData = mediumData.map((egg) => ({
    date: egg.date.toDate
      ? `${egg.date.toDate().getMonth() + 1}-${egg.date.toDate().getDate()}-${egg.date.toDate().getFullYear()}`
      : "",
    mediumCollected: egg.mediumCollected,
  }));
...
export default TotalMediumChart;
