...
function TotalPullets() {
  const [pulletData, setPulletData] = useState([]);
    ...
    const unsubscribe = onSnapshot(q, (querySnapshot) => {
      ...
      querySnapshot.forEach((doc) => {
        ...
        tempEggsData.push({
          ...
          pulletsCollected: eggData.pullets,
        });
      });
      setPulletData(tempEggsData);
    });

    return () => unsubscribe();
  }, [currentUser]);

  const formattedData = pulletData.map((egg) => ({
    date: egg.date.toDate
      ? `${egg.date.toDate().getMonth() + 1}-${egg.date.toDate().getDate()}-${egg.date.toDate().getFullYear()}`
      : "",
    pulletsCollected: egg.pulletsCollected,
  }));
