    ...
    const unsubscribe = onSnapshot(q, (querySnapshot) => {
      ...
      querySnapshot.forEach((doc) => {
        ...
        tempEggsData.push({
          ...
          largeCollected: eggData.large,
        });
      });
      setLargeData(tempEggsData);
    });

    return () => unsubscribe();
  }, [currentUser]);

  const formattedData = largeData.map((egg) => ({
    date: egg.date.toDate
      ? `${egg.date.toDate().getMonth() + 1}-${egg.date.toDate().getDate()}-${egg.date.toDate().getFullYear()}`
      : "",
    largeCollected: egg.largeCollected,
  }));
...
export default TotalLargeChart;
