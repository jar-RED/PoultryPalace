    ...
    const unsubscribe = onSnapshot(q, (querySnapshot) => {
      ...
      querySnapshot.forEach((doc) => {
        ...
        tempEggsData.push({
          ...
          smallCollected: eggData.small,
        });
      });
      setSmallData(tempEggsData);
    });

    return () => unsubscribe();
  }, [currentUser]);

  const formattedData = smallData.map((egg) => ({
    date: egg.date.toDate
      ? `${egg.date.toDate().getMonth() + 1}-${egg.date.toDate().getDate()}-${egg.date.toDate().getFullYear()}`
      : "",
    smallCollected: egg.smallCollected,
  }));
...
export default TotalSmallChart;
