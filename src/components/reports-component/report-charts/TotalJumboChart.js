    ...
    const unsubscribe = onSnapshot(q, (querySnapshot) => {
      ...
      querySnapshot.forEach((doc) => {
        ...
        tempEggsData.push({
          ...
          jumboCollected: eggData.jumbo,
        });
      });
      setJumboData(tempEggsData);
    });

    return () => unsubscribe();
  }, [currentUser]);

  const formattedData = jumboData.map((egg) => ({
    date: egg.date.toDate
      ? `${egg.date.toDate().getMonth() + 1}-${egg.date.toDate().getDate()}-${egg.date.toDate().getFullYear()}`
      : "",
    jumboCollected: egg.jumboCollected,
  }));
...
export default TotalJumboChart;
